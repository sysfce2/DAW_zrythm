...
Compilation options: -a data/plugins/zrythm-arch.cpp -lang cpp -fpga-mem-th 4 -ct 1 -cn highpass_filter -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0
------------------------------------------------------------ */
...
class highpass_filter : public dsp {
 ...
 public:
	...
	highpass_filter(const highpass_filter&) = default;
	
	virtual ~highpass_filter() = default;
	
	highpass_filter& operator=(const highpass_filter&) = default;
	
	void metadata(Meta* m) { 
		m->declare("author", "Zrythm");
		m->declare("compile_options", "-a data/plugins/zrythm-arch.cpp -lang cpp -fpga-mem-th 4 -ct 1 -cn highpass_filter -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0");
	...
	virtual highpass_filter* clone() {
		return new highpass_filter(*this);
	}
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp3 = highpass_filter_faustpower2_f(fTemp0);
...
namespace zrythm_faust
{
  ...
  std::unique_ptr<zrythm::plugins::faust::dsp> create_highpass_filter ()
  {
    return std::make_unique<highpass_filter> ();
  }
   ...
   * compute()).
   */
  void class_init_highpass_filter (int sample_rate)
  {
    ...
    if (initialized_sample_rate != sample_rate)
      {
        highpass_filter::classInit (sample_rate);
